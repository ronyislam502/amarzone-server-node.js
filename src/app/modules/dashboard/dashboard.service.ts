    ...
    Order.aggregate([
      { $match: { createdAt: dateFilter, isDeleted: { $ne: true } } },
      {
        $group: {
          ...
          totalRevenue: {
            $sum: {
              $cond: [{ $eq: ["$paymentStatus", PAYMENT_STATUS.PAID] }, "$grandAmount", 0],
            },
          },
          ...
          deliveredOrders: {
            $sum: {
              $cond: [
                { $in: ["$status", [ORDER_STATUS.DELIVERED, ORDER_STATUS.COMPLETE]] },
                1,
                0,
              ],
            },
...
const statisticsDashboardDataFromDB = async (
  ...
  switch (isUserExists.role) {
    ...
    case USER_ROLE.VENDOR:
      return getVendorDashboardFromDB(isUserExists._id as Types.ObjectId, query);
